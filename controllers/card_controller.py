...
@cards_bp.route('/<int:card_id>') # 8080/cards/4
def get_one_card(card_id): # example card_id = 4
    stmt = db.select(Card).filter_by(id=card_id) # select * from cards where id=4
    card = db.session.scalar(stmt)
    if card:
        return card_schema.dump(card)
    else:
        return {"Error": f"Card id {card_id} not found"}, 404
...
def delete_card(card_id):
    ...
    if card:
        ...
        db.session.commit()
        return {"msg": f"Card {card.title} deleted successfully"}
    else:
        return {"msg": f"Card id {card_id} not found"}, 404
